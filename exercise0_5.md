...
sequenceDiagram
    ...
    activate server
    server-->>browser: spa.js (The JavaScript file for the SPA)
    deactivate server
    ...
    activate server
    server-->>browser: [{ "content": "test", "date": "2026-06-26..." }, ... ]
    deactivate server

    Note right of browser: The browser executes the callback function that renders the initial notes using the DOM API